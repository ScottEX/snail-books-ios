...
iOS 端已经从 Capacitor 迁移到 **React Native + Expo 52**。**注意:web 端和 iOS 端现在是两套独立的代码,不再自动同步。** 详见下文"web → iOS 同步"章节。

- **登录页**:已可用,API 指向 `http://8.135.58.90:8601`
- **首页 / 业务屏**:暂时是 placeholder(显示"还在迁移"提示),等 web 端稳定后逐屏 RN 化
...
```
snail-books-ios/
├── App.tsx                 ← 入口,目前是 placeholder(详见 src/screens/HomeScreen.tsx 的迁移)
├── index.js                ← registerRootComponent + globals shim
├── app.json                ← Expo 配置
...
├── src/
│   ├── screens/            ← 7 个业务屏(web→RN 移植进行中)
│   ├── components/         ← 共享组件
│   ├── api/client.ts       ← iOS 端 API 客户端(默认指 8.135.58.90:8601)
│   ├── i18n.ts             ← i18n 字典
│   ├── theme.tsx           ← 主题(3 套)
│   ├── platform.ts         ← RN 平台 shim(localStorage / document / window / navigator)
│   └── globals.ts          ← 把 platform shim 挂到 globalThis 的副作用 import
...
- [ ] **placeholder → 真实 HomeScreen / 业务屏**
  - 现状: iOS 用 placeholder 顶住登录后界面
  - 需要: 把 web 6 个主屏 + 3 个二级屏改成 RN 原生版
  - 预计: 1-2 天(逐屏,每屏 1-3 小时)
  - 不在 web 任何 commit 里,是 iOS 长期欠债
